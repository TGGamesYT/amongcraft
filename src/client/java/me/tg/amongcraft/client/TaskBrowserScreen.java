...
import me.tg.amongcraft.TaskProgressTracker;
...
public class TaskBrowserScreen extends Screen {
    ...
    public TaskBrowserScreen(String url, BlockPos pos) {
        super(Text.literal("Task Browser"));

        // Get player session info
        var session = MinecraftClient.getInstance().getSession();
        String playerName = session.getUsername();
        String playerUUID = session.getUuid();
    ...
    protected void init() {
        ...
        if (browser == null) {
            ...
            browser = MCEF.createBrowser(url, transparent);
                ...
                Amongcraft.LOGGER.error("MCEF is not installed or failed to load.", e);
                MinecraftClient.getInstance().player.sendMessage(Text.of("MCEF not found!"), false);
            }
    ...
    private void markTaskAsDone(String taskId, BlockPos pos) {
        AmongcraftClient.TaskDoneC2SPacket.send(taskId, pos);
    }
    ...
    @Override
    public void close() {
        Amongcraft.LOGGER.info("Closing browser");
        browser.close();
        super.close();
    }
    ...
    public void render(DrawContext guiGraphics, int i, int j, float f) {
        ...
        if (browser.getURL().startsWith("amongcraft://taskdone/")) {
            String taskId = browser.getURL().substring("amongcraft://taskdone/".length());
            Amongcraft.LOGGER.info("Task done");
            markTaskAsDone(taskId, pos);
            close();
        } else if (browser.getURL().startsWith("amongcraft://taskfailed")) {
            Amongcraft.LOGGER.info("Task failed");
            close();
        }
    }
